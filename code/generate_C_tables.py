...
def security_loss_cosets(n, lam, q_s, sig, a):
    ...
    # Note that an adversary against the distribution P, may have a probability of only .5
    # 2^{-lambda} of finding a forgery, as some of its advantage may have been lost in the
    # reduction so far. Hence, assume at most 1 security is lost up to now, i.e.,
    # P(E) >= 2^-(lambda + 1), which then implies:
    # Q(E) >= P(E) P(E)^{1/(a-1)} / R_a(P || Q) >= P(E) / (2^{(lambda + 1)/(a-1)} R_a(P || Q)).
    return mpf('2')**((lam + 1) / (a - 1)) * renyi_div
